...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        for variable in self.crossword.variables:
            inconsistent_values = {word for word in self.domains[variable] if len(word) != variable.length}
    ...
    def revise(self, x, y):
        ...
        if overlap is not None:
            i, j = overlap
            values_to_remove = {value_x for value_x in self.domains[x] if all(value_x[i] != value_y[j] for value_y in self.domains[y])}
    ...
    def order_domain_values(self, var, assignment):
        ...
        def count_eliminations(value):
            return sum(1 for neighbor in self.crossword.neighbors(var) if value in self.domains[neighbor])
